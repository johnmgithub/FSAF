 ...
 *     b[i] = a[i]*a[i];
 * }
 * </code></pre> A serial version of a similar function for 2D arrays is:
 * <pre><code>
 ...
 *     sqr(a[i],b[i]);
 * }
 * </code></pre> Using this class, the parallel version for 2D arrays is:
 * <pre><code>
 ...
 *   });
 * }
 * </code></pre> In the parallel version, the method {@code compute} defined by
 * the interface {@code LoopInt} will be called n times for different indices i
 * in the range [0,n-1]. The order of indices is both indeterminant and
 * irrelevant because the computation for each index i is independent. The
 * arrays a and b are declared final as required for use in the implementation
 * of {@code LoopInt}.
 * <p>
 ...
 * </code></pre> A similar method facilitates tasks that reduce a sequence of
 * indexed values to one or more values. For example, given the following
 * method:
 * <pre><code>
 ...
 *   return s;
 * }
 * </code></pre> serial and parallel versions for 2D arrays may be written as:
 * <pre><code>
 ...
 *   });
 * }
 * </code></pre> In the parallel version, we implement the interface
 * {@code ReduceInt} with two methods, one to {@code compute} sums of array
 * elements and another to {@code combine} two such sums together. The same
 * pattern works for other reduce operations. For example, with similar
 * functions we could compute minimum and maximum values (in a single reduce)
 * for any indexed sequence of values.
 * <p>
 ...
 * </code></pre> The methods loop and reduce require that begin is less than end
 * and that step is positive. The requirement that begin is less than end
 * ensures that reduce is always well-defined. The requirement that step is
 * positive ensures that the loop terminates.
 * <p>
...
public class Parallel {
     ...
     *   }
     * });
     * </code></pre> This wrapper is most useful when (1) the cost of
     * constructing an unsafe object is high, relative to the cost of each call
     * to compute, and (2) the number of threads calling compute is
     * significantly lower than the total number of such calls. Otherwise, if
     * either of these conditions is false, then simply construct a new unsafe
    ...
    public static <V> V reduce(
        ...
        if (_serial || end <= begin + chunk * step) {
            ...
            ReduceIntTask<V> task = new ReduceIntTask<>(begin, end, step, chunk, body);
     ...
     * <p>
     * <em>Setting this flag to false disables parallel processing for all users
     * of this class.</em> This method should therefore be used for testing and
     * benchmarking only.
     *
