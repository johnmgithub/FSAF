 ...
 * @author G. W. Stewart
 * 
 * Modified by John Mulcahy 2024 to allow re-use of an SVD instance for processing 
 * another matrix of the same size without reallocating the U, S, V matrices and to
 * allow the argument matrix to be overwritten.
 */
public final class Zsvd {
    ...
    /**
     * Allows reuse of U,V,S matrices if the next matrix to be processed is
     * the same size.
     * @param XX a Zmat
     * @param preserveXX if true make a copy of XX else modify XX
     */
    public void setXX(Zmat XX, boolean preserveXX){
        int mc = Math.min(XX.nr, XX.nc);
        if (U == null || U.nr != XX.nr){
            U = Eye.o(XX.nr);
        }else{
            U.eye();
        }
        if (V == null || V.nr != XX.nc){
            V = Eye.o(XX.nc);
        }else{
            V.eye();
        }
        if (S == null || S.order != mc){
            S = new Zdiagmat(mc);
        }else{
            S.reset();
        }
        Zmat X = preserveXX ? new Zmat(XX) : XX;
        calculate(X, mc);
    }
     ...
     *                Passed from below.
     */
    public Zsvd(Zmat XX, boolean preserveXX) throws ZException {
        int mc = Math.min(XX.nr, XX.nc);
        S = new Zdiagmat(mc);
        U = Eye.o(XX.nr);
        V = Eye.o(XX.nc);
    ...
    private void calculate(Zmat X, int mc){

        int i, il, iu, iter, j, k, kk, m;
