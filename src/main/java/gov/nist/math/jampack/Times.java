...
public final class Times {
    ...
    public static Zmat o(Zmat A, Zmat B) throws ZException {
        if (A.nc != B.nr) {
            throw new ZException("Unconformity in product, A:"+A.nr+"X"+A.nc+", B:"+B.nr+"X"+B.nc);
        }
     ...
     * @throws ZException 
     */
    public static Z1 o(Zmat A, Z1 B) throws ZException {
        if (A.nc != B.n) {
            throw new ZException("Unconformity in product, A:"+A.nr+"X"+A.nc+", B:"+B.n);
        }
        Z1 C = new Z1(A.nr);
        for (int i = 0; i < A.nr; i++) {
            for (int k = 0; k < A.nc; k++) {
                ...
                double imik = A.im(i, k);
                double reik = A.re(i, k);
                C.addRe(i, reik * rek - imik * imk);
                C.addIm(i, imik * rek + reik * imk);
            }
        }
        return C;
    }
