...
public class JamaMatrix implements Cloneable, java.io.Serializable {
    ...
    public JamaMatrix copy() {
        ...
        double[][] C = X.getArray();
        for (int i = 0; i < rows; i++) {
            System.arraycopy(A[i], 0, C[i], 0, cols);
        }
        return X;
    }
    ...
    public double[] getRowPackedCopy() {
        double[] vals = new double[rows * cols];
        for (int i = 0; i < rows; i++) {
            System.arraycopy(A[i], 0, vals, i * cols, cols);
        }
        return vals;
    }
    ...
    /**
     * Product of a diagonal matrix D and this matrix.
     * @param D the elements of a diagonal matrix
     * @return (Matrix D).times(this)
     */
    public JamaMatrix diagTimes(double[] D) {
        if (D.length != rows) {
            throw new IllegalArgumentException("Matrix inner dimensions must agree");
        }
        JamaMatrix B = new JamaMatrix(rows, cols);
        for (int i = 0; i < rows; i++) {
            double rei = D[i];
            for (int j = 0; j < cols; j++) {
                B.A[i][j] = rei * A[i][j];
